...
PDDR_All_FCoXX$Analyte = factor(PDDR_All_FCoXX$Analyte, levels =c("FCoXX_M2_Mek", "FCoXX_M2_cRaf")) # This is to get the correct sequence in the Facet_wrap
Analyte_labels <- c("FCoXX_M2_Mek" = "pMEK", "FCoXX_M2_cRaf" = "pRAF1")
PDDR_All_FCoXX$Cell_line = factor(PDDR_All_FCoXX$Cell_line, levels =c("XX", "XO")) # This is to get the correct sequence in the Facet_wrap
...
g <- ggplot(Fig7G_pMek, aes(x=x_axis_factor,y=Signal)) +
  ...
  ylim(0,max(Fig7G_pMek$Signal)+0.5)+
  labs(x = "\n MEKi(nM)", 
       y = " Rel. phosp. (norm.) ",
       title = "pMEK",
       color = "Cell line" )+
...
grid.arrange(gt)
ggsave("Fig7G_pMek.pdf", gt, dpi=300, useDingbats=FALSE, path = "./OUTPUT_PAPER") 
...
Fig7G_Mek_Replicates = Fig7G_pMek %>% 
...
Fig7G_Mek_data = left_join(Fig7G_Mek_Replicates,Fig7G_Mek_compared_groups_ttest, by=c("Treatment","Cell_line")) %>% 
  arrange(desc(Cell_line)) %>% 
  mutate(Analyte= gsub("FCoXX_M2_","",Analyte))

########### Fig 7G : cRaf ###########

Fig7G_pcRaf = PDDR_All_FCoXX_SUBSET %>% 
  filter(Analyte == "FCoXX_M2_cRaf") %>% 
  filter(Treatment != 1000) %>% 
  filter(!(Cell_line == "XX" & (Treatment == 4|Treatment == 12)))

g <- ggplot(Fig7G_pcRaf, aes(x=x_axis_factor,y=Signal)) +
  geom_point(aes(x=x_axis_factor,y=Signal,color=Cell_line))+
  scale_fill_manual(values = MyCellLineColours) +
  scale_color_manual(values = MyCellLineColours) +
  stat_summary(fun = mean,geom = "crossbar", size = 0.1, position = position_dodge(width = 1))+
  ylim(0.5,max(Fig7G_pcRaf$Signal)+0.5)+
  labs(x = "\n cRafi(nM)", 
       y = " Rel. phosp. (norm.) ",
       title = "pcRaf",
       color = "Cell line" )+
...
gt=set_panel_size(g,width=unit(2.8,'cm'),height=unit(2.8,'cm'))
grid.arrange(gt)
ggsave("Fig7G_pcRaf.pdf", gt, dpi=300, useDingbats=FALSE, path = "./OUTPUT_PAPER") 

# Test for significance of results : Fig7G_cRaf

cRaf_XX_0= Fig7G_pcRaf %>% 
  filter(Cell_line=="XX" & Treatment_Fctr ==0) 

cRaf_XO_0 = Fig7G_pcRaf %>% 
  filter(Cell_line=="XO" & Treatment_Fctr == 0) 

cRaf_XO_4 = Fig7G_pcRaf %>% 
  filter(Cell_line=="XO" & Treatment_Fctr == 4)

cRaf_XO_12 = Fig7G_pcRaf %>% 
  filter(Cell_line=="XO" & Treatment_Fctr == 12) 


Fig7G_cRaf_compared_groups=list("cRaf_XO_0"=cRaf_XO_0,"cRaf_XO_4"=cRaf_XO_4,"cRaf_XO_12"=cRaf_XO_12)
Fig7G_cRaf_compared_groups_ttest=list()

for(i in 1:length(Fig7G_cRaf_compared_groups)) {
  
  Fig7G_cRaf_compared_groups_ttest$compared_group[i] = names(Fig7G_cRaf_compared_groups[i])
  
  unpaired_Ttest = t.test(cRaf_XX_0$Signal,Fig7G_cRaf_compared_groups[[i]]$Signal, paired = FALSE)
  
  Fig7G_cRaf_compared_groups_ttest$XX_untreated_mean[i] = unpaired_Ttest$estimate[1]
  Fig7G_cRaf_compared_groups_ttest$XO_treated_mean[i] = unpaired_Ttest$estimate[2]
  Fig7G_cRaf_compared_groups_ttest$p_value[i] = unpaired_Ttest$p.value
  
}

Fig7G_cRaf_compared_groups_ttest <- data.frame((sapply(Fig7G_cRaf_compared_groups_ttest,c)))
Fig7G_cRaf_compared_groups_ttest$p_value = as.numeric(as.character(Fig7G_cRaf_compared_groups_ttest$p_value))
Fig7G_cRaf_compared_groups_ttest <- Fig7G_cRaf_compared_groups_ttest %>% 
  mutate(sig = ifelse(p_value<0.05,"*","")) %>% 
  separate(compared_group, into = c("Analyte1","Cell_line","Treatment"), sep = "_", fill = "right") %>% 
  select(-c(Analyte1)) %>% 
  mutate(Treatment=as.numeric(as.character(Treatment)))

Fig7G_cRaf_Replicates = Fig7G_pcRaf %>% 
  select(Cell_line,Replicate,Treatment,log2Treatment,Analyte,Signal) %>% 
  mutate(Treatment=as.numeric(as.character(Treatment))) %>%
  pivot_wider(names_from = Replicate,values_from=Signal) %>% 
  rowwise() %>% 
...
Fig7G_cRaf_data = left_join(Fig7G_cRaf_Replicates,Fig7G_cRaf_compared_groups_ttest, by=c("Treatment","Cell_line")) %>% 
  arrange(desc(Cell_line)) %>% 
  mutate(Analyte= gsub("FCoXX_M2_","",Analyte))


########### Fig 7G : writing out the data as xls ###########
Fig7G_Mek_cRaf_data = dplyr::bind_rows(Fig7G_Mek_data,Fig7G_cRaf_data)
WriteXLS::WriteXLS(Fig7G_Mek_cRaf_data, "./OUTPUT_PAPER/Fig7G_Mek_cRaf_data.xls")


######################## FIG 7H ###############

Fig7H_pMek = PDDR_All_FCoXX_SUBSET %>% 
  filter(Analyte == "FCoXX_M2_Mek") %>% 
  filter(Treatment != 12) %>% 
...
g <- ggplot(Fig7H_pMek, aes(x=x_axis_factor,y=Signal)) +
  geom_point(aes(x=x_axis_factor,y=Signal,color=Cell_line))+
  scale_fill_manual(values = MyCellLineColours) +
  scale_color_manual(values = MyCellLineColours) +
  stat_summary(fun = mean,geom = "crossbar", size = 0.1, position = position_dodge(width = 1))+
  ylim(0,max(Fig7H_pMek$Signal)+0.5)+
  labs(x = "\n MEKi(nM)", 
       y = " Rel. phos. (norm) ",
       title = "pMEK",
       color = "Cell line" )+
...
gt=set_panel_size(g,width=unit(2.8,'cm'),height=unit(2.8,'cm'))
grid.arrange(gt)
ggsave("Fig7H_Mek.pdf", gt, dpi=300, useDingbats=FALSE, path = "./OUTPUT_PAPER") 
...
for(i in 1:length(the_cell_lines)){
  ...
  Fig7H_ttest$base_conc[i] = unique(base$Treatment)
  Fig7H_ttest$treated_conc[i] = unique(treated$Treatment)
...
Fig7H_Mek_Replicates = Fig7H_pMek %>% 
  select(Cell_line,Replicate,Treatment,log2Treatment,Analyte,Signal) %>% 
  mutate(Treatment=as.numeric(as.character(Treatment))) %>%
  pivot_wider(names_from = Replicate,values_from=Signal) %>% 
  rowwise() %>% 
...
Fig7H_Mek_data = left_join(Fig7H_Mek_Replicates,Fig7H_ttest, by=c("Treatment","Cell_line")) %>% 
  arrange(desc(Cell_line)) %>% 
  mutate(Analyte= gsub("FCoXX_M2_","",Analyte)) %>%
...
WriteXLS::WriteXLS(Fig7H_Mek_data, "./OUTPUT_PAPER/Fig7H_Mek_data.xls")
