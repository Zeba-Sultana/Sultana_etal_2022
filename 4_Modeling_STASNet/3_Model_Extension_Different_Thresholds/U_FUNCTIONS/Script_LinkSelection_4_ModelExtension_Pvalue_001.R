...
Select_Link<- function(XX_ext_list, XO_ext_list, Selection_Criterion ) {
  ...
  XX_CL_Rank = select(XX_ext_ordered, c(Link, Rank, Res_delta, adj_pval,Perc_Res_delta))
  XO_CL_Rank = select(XO_ext_ordered, c(Link, Rank, Res_delta, adj_pval,Perc_Res_delta))
